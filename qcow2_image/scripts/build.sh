...
# Step 2: Partition the disk (including BIOS Boot Partition)
...
# Step 3: Format the boot and root partitions
...
# Step 5: Setup bootloader files on the boot partition
sudo cp -p "./target/kernel/build/init_ram_fs/boot/starship" "$BOOT_MOUNT/starship"
sudo cp -rp "./target/kernel/build/init_ram_fs/lib" "$BOOT_MOUNT"
sudo cp -p "./target/ramdisks/build/initramfs.hd0.gz" "$BOOT_MOUNT"
sudo mkdir -p "$BOOT_MOUNT/grub"
sudo cp -p "./target/grub/hd0/init_ram_fs/boot/grub.cfg" "$BOOT_MOUNT/grub/grub.cfg"

# Install GRUB on the device
log "Installing GRUB..."
sudo grub-install --target=i386-pc --boot-directory="$BOOT_MOUNT" "$LOOP_DEVICE"

# Step 6: Populate the root filesystem
